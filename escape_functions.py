...
def better_diffusion(fH2O,T,g,fCO2,fO2,fN2): # diffusion H2O thrugh CO2 and N2
    ...
    return phi/6.022e23 #mol H2O/m2/s 

@jit(nopython=True)
def better_H_diffusion(fH,T,g,fCO,fO,fN2): # diffusion H2O thrugh CO2 and N2
    pO = fO
    pN2 = fN2
    pCO = fCO
    ## Units conversion 
    ...
    #bH2O_CO2 = 100 * 9.24e-5/(1.38e-22) * T**0.5 / np.exp(307.9/T) # conversion m
    #bH2O_N2 = 100 * 0.187e-5/(1.38e-22)  * T**1.072 # conversion m 
    #bH2O_O2 = 100 *  0.189e-5/(10*1.38e-23) * T**1.072 # conversion m -> 0.189e-5/(10*1.38e-23) = 1.37e16
    bH_O = 100*4.8e17*T**0.75 #Zahnle 1986 need to check
    bH_N2 = 100* 6.5e17 * T**0.7 #Zahnle 1986 need to check
    bH_CO = 100 * 6.5e17 * T**0.7 #Zahnle 1986 need to check
    bH = (bH_CO * pCO + bH_N2 * pN2 + bH_O*pO) / (pCO + pN2 + pO)
    Mn = (0.028 * pCO + 0.028 * pN2 + 0.016 * pO)/(pCO + pN2 + pO) #kg/mol
    
    Hn = (8.314*T)/(Mn*g)
    H_H = (8.314*T)/(0.001*g)
    phi = bH * fH * (1.0/Hn - 1.0/H_H)
    return phi/6.022e23 #mol H/m2/s 
...
def find_epsilon(T,RE,ME,FXUV, XO, XH, XC,epsilon_init,mixparam):
    ...
    if ratio > 1.0:
        ...
        return ((1-mixparam)*epsilon + mixparam*epsilon_init) # mix param = 0.9 -> only 10% energy above xj=1 goes into extra escape
...
FXUV_ar = np.logspace(-3,2,100)
a_ar = np.copy(FXUV_ar)
b_ar = np.copy(FXUV_ar)
c_ar = np.copy(FXUV_ar)
for i in range(0,len(FXUV_ar)):
    [a,b,c,d] = Odert_three(400,1,1,0.3,FXUV_ar[i], 1/3, 2/3, 0.0)
    a_ar[i] = a
    b_ar[i] = b
    c_ar[i] = c

plt.figure()
plt.loglog(FXUV_ar,a_ar/mH,'b')
plt.loglog(FXUV_ar,b_ar/mO,'r')
plt.loglog(FXUV_ar,c_ar/mO,'k--')
plt.show()
'''
